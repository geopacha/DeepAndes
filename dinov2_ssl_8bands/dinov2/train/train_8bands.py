...
def get_args_parser(add_help: bool = True):
    ...
    Key Args:
        --config-file: /path/to/ssl_training/config/file/xxx.yaml 
        --output-dir:  /path/to/store ckpt and logs 
        --ssl-data:    this is added, which is /path/to/ssl_data/folder
        --wandb-trial: this is added, which is the name of the "run"
...
def do_train(cfg, model, resume=False, main_args=None):
    ...
    # change: allow for multi channels
    data_transform = DataAugmentationDINO_MS(
    ...
    for data in metric_logger.log_every(
        ...
        wandb.log({
            ...
            "weight decay": wd,
            "momentum": mom,
            "total_loss": round(losses_reduced, 4),
            "dino_local_crops_loss": round(loss_dict_reduced['dino_local_crops_loss'], 4),
            "dino_global_crops_loss": round(loss_dict_reduced['dino_global_crops_loss'], 4),
            "ibot_loss": round(loss_dict_reduced["ibot_loss"], 2),
            "koleo_loss": round(loss_dict_reduced["koleo_loss"])
                   })
...
def main(args):
    ...
    do_train(cfg, model, resume=not args.no_resume, main_args=args) #add fix: args 
...
if __name__ == "__main__":
    ...
    run = wandb.init(project=project, name=trial,
                config=vars(args))
    main(args)
